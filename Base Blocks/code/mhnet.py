...
class MHNet(nn.Module):
    """
    Single encoder block from MHNet.
    Keeps out_channels = in_channels and output image size same as input.
    No downsampling or skip connection, just one BotBlock as in MHNet encoder.
    """
    def __init__(self, in_channels=3,dummy=0):
        super().__init__()
        self.isOk = True
        self.botblock = BotBlock(in_channels)
    def forward(self, x):
        return self.botblock(x)